...
import {
  ...
  signMentor,
  signStartup,
} from "../features/mentoringSessions/mentoringSessionSlice";
...
const MentoringSessionList = ({ userId, role }) => {
  ...
  if (sessions.length === 0) return <p>No hay sesiones disponibles.</p>;
    ...
    <div className="p-4 space-y-4">
      ...
      {sessions.map((session) => (
        <div key={session._id} className="border p-4 rounded shadow-sm">
          ...
          <p>
            <strong>Startup:</strong> {session.startup?.participant}
          </p>
          <p>
